...
export async function generateStaticParams() {
    ...
    allBlogs.map((blog) => {
        if (blog.isPublished) {
            blog.tags.map((tag) => {
                let slugged = slugger.slug(tag);
                if (!categories.includes(slugged)) {
                    categories.push(slugged);
                    paths.push({ slug: slugged });
                }
            });
        }
    });
...
export const CategoryPage = ({ params }) => {
    ...
    const blogs = allBlogs.filter((blog) => {
        return blog.tags.some((tag) => {
            ...
            if (!allCategories.includes(slugged)) {
                allCategories.push(slugged);
            }
            if (params.slug === "all") {
                return true;
            }
